...
typedef struct mos_t_idx_ops {
   uint64_t (*get_index_size)(const uint64_t item_count);
   void (*init_index)(const uint64_t item_count, mos_t_idx* idx, mos_t_idx_data* idx_data);
   int64_t (*put)(const mos_t_idx_data* idx_data, const uint8_t* key, const size_t key_len, const uint64_t value);
   ...
   void (*remove)(const mos_t_idx_data* idx_data, const uint8_t* key, const size_t key_len);
} mos_t_idx_ops;
...
static void mos_idx_remove_value(const MOS_IDX_TYPE idx_type, const mos_t_idx_data* idx_data, const uint8_t* key, const size_t key_len) {
    mos_t_idx_ops idx_ops = MOS_IDX_OPS_REGISTRY[idx_type];
