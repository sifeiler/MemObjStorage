...
uint64_t mos_idx_data_size(mos_t_config* config) {
    ...
    for (uint64_t i = 0; i < index_count; i++) {
        ...
        total_idx_size += idx_ops.get_index_size(config->max_records);
    }
...
void mos_idx_create(const mos_t_storage* storage, mos_t_config* mos_config) {
    ...
    uint64_t offset_index_data = header->layout.offset_index_data;
    for (uint64_t i = 0; i < header->index_count; i++) {
        mos_t_idx* curr_index = index + i;
        mos_t_idx_data* index_data = MOS_GET_PTR(storage->mmap_ptr, offset_index_data);
        curr_index->offset_file = offset_index_data;
        mos_idx_init(header->max_records, curr_index, index_data);

        memcpy(&index_data->index, curr_index, sizeof(mos_t_idx));

        //new offset is old offset + the index data stored at old offset
        offset_index_data += index->index_size;
    }
}
...
void mos_idx_put(const MOS_IDX_TYPE idx_type, const mos_t_idx_data* idx_data, const uint8_t* key, const size_t key_len, const uint64_t value) {
    mos_t_idx_ops idx_ops = MOS_IDX_OPS_REGISTRY[idx_type];
    idx_ops.put(idx_data, key, key_len, value);
}
